...
class AccountController extends BaseController{
	...
	public function postEditProfile(){
		...
		echo $user_password;
		echo '<br>';
		$user_input_password = Input::get('pass1');
		$user_input_password_hashed = Hash::make($user_input_password);
		echo $user_input_password_hashed;

		dd();

		if($user_password == Hash::make($user_input_password)){
			// edit & save profile data
				$validator = Validator::make(Input::all(), 
					array(
						//
						// kaip padaryti kad galetu ir nepakeite uzseivinti ???
						//
						'email'=>'required|max:50|unique:users',
						'username'=>'required|max:20|min:3|unique:users',
						'password'=>'required|min:6',
				...
				if($validator->fails()){
					...
					if($user->save()) {
						return Redirect::route('edit-profile')
						->with('global', 'Your account details has changed successfully!');
					}
				}

		}else{
			return Redirect::route('edit-profile')->with('global', 'Enter valid password before change something!');
		}


		// if($user = User::where('password', '=', Input::get(Hash::make($password)))){
		// 	dd('COOL!');
		...
		//return Redirect::route('edit-profile')->with('global', 'It\'s ok!');
	}
}
